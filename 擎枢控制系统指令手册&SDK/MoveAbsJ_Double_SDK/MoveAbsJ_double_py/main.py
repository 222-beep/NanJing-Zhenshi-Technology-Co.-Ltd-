...
def main():
    ...
    # 发送初始化指令  
    arch.send_rpcsy(client, init_cmds, 500, 0.1)  # 同步rpc (client, 指令, 超时时间ms, 休眠时间s)
    
    # 主循环发送运动指令
    while True:
        # 发送同步双臂运动指令
        arch.send_rpcsy(client, motion_cmds, 5000, 0.5)
        # 发送异步双臂在线规划指令
        arch.send_rpc_async(client, motion_speedL_cmds, 10000, 0.5)
