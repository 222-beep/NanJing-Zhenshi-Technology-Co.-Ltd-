...
namespace core
{
  ...
  struct MsgHeader
  {
    ...
    MsgType msg_type_;       /* 1-需要应答的请求/2-应答/101-无需应答的命令/其他 */
  ...
  {
  public:
    auto virtual resize(MsgSize size) -> int = 0;
    ...
    auto msgBornTime() const -> int64_t { return header().reserved2_; }
    auto setMsgReturnTime(const int64_t &return_ts) -> void { header().reserved3_ = return_ts; }
    auto msgReturnTime() const -> int64_t { return header().reserved3_; }
    /* [END] */
  ...
  public:
    ...
    auto virtual resize(MsgSize data_size) -> int override;
  ...
  public:
    ...
    explicit MsgFix(MsgID msg_id = 0, MsgSize size = 0) : MsgBase()
    {
      resize(size);
      setMsgID(msg_id);
    }
